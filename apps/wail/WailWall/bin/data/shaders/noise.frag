...
float pattern( in vec2 p, out vec2 q, out vec2 r )
{
	...
	q.y = pnoise2D( p + vec2(5.2,1.3) );

	r.x = pnoise2D( p + 4.0*q + vec2(1.7,9.2) );
	r.y = pnoise2D( p + 4.0*q + vec2(8.3,2.8) );

	return pnoise2D( p + 4.0*r );
}
*/
...
//	vec2 q,r;
//	pattern(gl_TexCoord[0].st/250., q, r);
//	gl_FragData[0] = vec4(q.x,q.y,r.y,1.0);
