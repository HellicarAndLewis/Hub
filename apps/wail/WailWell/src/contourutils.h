...
namespace tricks {
	namespace math {
		...
		int findLeftmostPoint(vector<ofVec2f> &points);
		int findRightmostPoint(vector<ofVec2f> &points);
		int findTopmostPoint(vector<ofVec2f> &points);
		int findBottommostPoint(vector<ofVec2f> &points);
		/**
		...
		/**
		 * Tells you the minimum distance between two contours, the precision is how many points
		 * to jump each time when comparing. Returns FLT_MAX if there's a problem.
		 */
		float getClosestDistanceBetweenTwoContours(const vector<ofVec2f> &a, const vector<ofVec2f> &b, int precision = 1);
		float getClosestDistanceBetweenTwoContours(const vector<ofVec3f> &a, const vector<ofVec3f> &b, int precision = 1);
	}
}
