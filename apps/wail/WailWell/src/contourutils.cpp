...
int tricks::math::findLeftmostPoint(vector<ofVec2f> &points) {
	int minI = -1;
	float minX = FLT_MAX;
	for(int i = 0; i < points.size(); i++) {
		if(points[i].x<minX) {
...
int tricks::math::findRightmostPoint(vector<ofVec2f> &points) {
	int minI = -1;
	float maxX = FLT_MIN;
	for(int i = 0; i < points.size(); i++) {
		if(points[i].x>maxX) {
...
int tricks::math::findTopmostPoint(vector<ofVec2f> &points) {
	int minI = -1;
	float minY = FLT_MAX;
	for(int i = 0; i < points.size(); i++) {
		if(points[i].y<minY) {
...
int tricks::math::findBottommostPoint(vector<ofVec2f> &points) {
	int minI = -1;
	float minY = FLT_MIN;
	for(int i = 0; i < points.size(); i++) {
		if(points[i].y>minY) {
...
float tricks::math::getClosestDistanceBetweenTwoContours(const vector<ofVec2f> &a, const vector<ofVec2f> &b, int precision) {
	if(precision<1) {
		printf("tricks::math::getClosestDistanceBetweenTwoContours() - precision is smaller than 1, cannot calculate.\n");
		return FLT_MAX;
	}
	float minDistSqr = FLT_MAX;
	for(int i = 0; i < a.size(); i += precision) {
		for(int j = 0; j < b.size(); j += precision) {
			float distSqr = a[i].distanceSquared(b[j]);
...
float tricks::math::getClosestDistanceBetweenTwoContours(const vector<ofVec3f> &a, const vector<ofVec3f> &b, int precision) {
	if(precision<1) {
		printf("tricks::math::getClosestDistanceBetweenTwoContours() - precision is smaller than 1, cannot calculate.\n");
		return FLT_MAX;
	}
